...
```{r load rds}
AS.integrated <- readRDS("../AS.integrated.rds")
```
...
AS.integrated <- subset(AS.integrated, idents = c(0,4), invert = TRUE) # we are removing clusters 0,4 because they have very low average UMI/feature counts in comparison to other clusters, 4 is also found almost exclusively in a single sample (AS_TX_skin), so I think it's garbage. Cluster 0 is expressing mostly ribosomal genes.
```
...
DefaultAssay(AS.integrated) <- "RNA"
```
...
AS.list <- SplitObject(AS.integrated, split.by = "orig.ident")
...
#For example:
DefaultAssay(object = AS.list$AS_TA_skin)
```
...
features <- SelectIntegrationFeatures(object.list = AS.list, nfeatures = 5000, normalization.method = "SCT")
```
...
AS.list <- PrepSCTIntegration(object.list = AS.list, anchor.features = features)
...
anchors <- FindIntegrationAnchors(object.list = AS.list, anchor.features = features, normalization.method = "SCT", reduction = "rpca")
```
...
saveRDS(AS.integrated, file = "AS.integrated_filter1.rds")
