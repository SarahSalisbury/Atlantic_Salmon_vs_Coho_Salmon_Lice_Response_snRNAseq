...
```{r load rds}
#read in the .rds after first filtration (using lower PCs)
CO.integrated <- readRDS("../3_Filter1/CO.integrated_filter1.rds")
```
...
CO.integrated <- subset(CO.integrated, idents = c(11), invert = TRUE) # we are removing cluster 11 because all of it's marker genes are ribosomal genes :(
```
...
png("UMAP_clusters_separated_origin_names_on_cells_filter2.png", res=600, width=6200, height=3200)
...
png("UMAP_cellcycle_filter2.png", res=600, width=4200, height=3200)
