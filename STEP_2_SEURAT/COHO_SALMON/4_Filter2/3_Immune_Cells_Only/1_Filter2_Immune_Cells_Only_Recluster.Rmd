---
title: "Filter2 Immune Cells Only Recluster"
...
# Now please add PrepSCTFindMarkers() because this hasn't yet been run on the integrated seurat object
...
# So we just want to look at the Myeloid and T-cell clusters (5, 6 respectively)
CO.integrated <- subset(CO.integrated, idents = c(5,6), invert = FALSE) # we are just keeping clusters 5 and 6 (immune clusters).
```
...
CO.integrated <- RunPCA(CO.integrated, dims = 1:20) # Please note, "dims" is not a valid argument for RunPCA, therefore the default 50 PCs were generated here in contrast to what was reported in the "Sub-Clustering" section of the Methods in the manuscript.
```
...
ElbowPlot(CO.integrated, ndim = 20)
#So probably 15 PCs is ok

# 3. Make a UMAP
# use the number of dimensions as determined from Elbowplot
CO.integrated <- RunUMAP(CO.integrated, dims = 1:15, verbose = FALSE)
...
CO.integrated <- FindNeighbors(CO.integrated, dims = 1:15)
...
CO.integrated <- FindClusters(CO.integrated, resolution = 0.4) ###Potentially play with "resolution = 0.5"
```
```{r plot UMAPs, message = FALSE}
#You can save the object at this point so that it can easily be loaded back in without having to rerun the computationally intensive steps performed above, or easily shared with collaborators.
saveRDS(CO.integrated, file = "CO.integrated_filter1_immuneonly.rds")
...
png("UMAP_clusters_names_on_cells_filter2_immune.png", res=600, width=4200, height=3200)
DimPlot(CO.integrated, reduction = "umap", label = TRUE, pt.size = 0.1) + NoLegend()
dev.off()

# Plot UMAP with cells coloured by sample origin
png("UMAP_origin_filter2_immune.png", res=600, width=4200, height=3200)
DimPlot(CO.integrated, group.by = "orig.ident", pt.size = 0.1)
dev.off()

# Plot UMAP with clusters in a legend for each sample
png("UMAP_clusters_separated_origin_filter2_immune.png", res=600, width=6200, height=3200)
...
png("UMAP_clusters_separated_origin_names_on_cells_filter2_immune.png", res=600, width=6200, height=3200)
...
png("UMAP_cellcycle_filter2_immune.png", res=600, width=4200, height=3200)
