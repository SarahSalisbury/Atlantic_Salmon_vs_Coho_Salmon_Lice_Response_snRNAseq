---
title: "Filter2 Filter Differential Expression Results"
...
for( i in seq_along(ldfnobad)){
  ...
  ldfnobad[[i]]$CellID <- gsub("DE-CO_","",ldfnobad[[i]]$CellID) # remove DE_CO from CellID column
  ...
  ldfnobad[[i]]$TimeID <- gsub("^.*T","T",ldfnobad[[i]]$TimeID) # remove everything up to the first T in TimeID column and then replace the T
}
```

```{r}
new.cluster.ids <- c("0" = "Intermediate Keratinocytes",
...
# Reset idents
Idents(CO.integrated) <- "celltype"

# Get number of cells per cluster and per sample of origin
#https://github.com/satijalab/seurat/issues/738
cells_per_cluster_per_sample <- table(CO.integrated@meta.data$celltype, CO.integrated@meta.data$TIME)
...
for(i in 1:length(split_data_nolow)){
  write.table(split_data_nolow[[i]], paste0("NOLOW_",names(split_data_nolow)[i], ".txt"), col.names = TRUE, row.names = FALSE, quote = FALSE, sep = ",")
}
```
