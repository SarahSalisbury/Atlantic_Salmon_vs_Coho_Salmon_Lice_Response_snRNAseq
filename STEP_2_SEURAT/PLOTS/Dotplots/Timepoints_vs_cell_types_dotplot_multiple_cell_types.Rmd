---
title: "Timepoints vs Cell Types Dotplot - Multiple Cell Types"
...
Dotplot_CO<- function(genetoplot, listofcelltypes) {
  #Get data
   ...
   #https://divingintogeneticsandgenomics.rbind.io/post/clustered-dotplot-for-single-cell-rnaseq/
    ...
    row.names(exp_mat) <- exp_mat$time  # make times the row names
    exp_mat <- exp_mat[,-1] %>% as.matrix() #delete first column (time column)
    exp_mat <- t(exp_mat)
    ...
    row.names(percent_mat) <- percent_mat$time  # make times the row names
    percent_mat <- percent_mat[,-1] %>% as.matrix() #delete first column (time column)
  ...
  #plot
    ...
    hp<- Heatmap(exp_mat,
                 ...
                 border = "black",
                 row_order = order_cells_CO[order_cells_CO %in% codelistofcelltypes],
                 column_order = rev(order_time_CO),
                 cluster_rows = FALSE,
                 ...
                 column_labels = c("Control", "12h", "24h", "36h", "48h", "60h"), # please make sure this order matches the order of exp_mat$time - does not necessarily = order_time_CO, instead it seems to be the order the times are in your metadata, so always TX, TA, TB, TC, TD, TE!
                 row_labels = new.cluster.ids_CO[rownames(exp_mat)],
...
Dotplot_AS<- function(genetoplot, listofcelltypes) {
  #Get data
  ...
  #plot
    ...
    hp<- Heatmap(exp_mat,
                 ...
                 column_names_gp = gpar(fontsize = 12),
                 border = "black",
                 row_order = order_cells_AS[order_cells_AS %in% codelistofcelltypes],
                 column_order = rev(order_time_AS),
                 cluster_rows = FALSE,
                 cluster_columns = FALSE,
                 width = ncol(exp_mat)*unit(4, "mm"), 
                 height = nrow(exp_mat)*unit(4, "mm"), #https://jokergoo.github.io/2020/05/11/set-cell-width/height-in-the-heatmap/
                 column_labels = c("Control", "12h", "24h", "36h", "48h", "60h"), # please make sure this order matches the order of exp_mat$time - does not necessarily = order_time_AS, instead it seems to be the order the times are in your metadata, so always TX, TA, TB, TC, TD, TE!
                 row_labels = new.cluster.ids_AS[rownames(exp_mat)],
                 #https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html
                 row_names_side = "left"
                 ) 
    nice_name <- gsub(":","",genetoplot)
...
```{r}
celllist <- c("Suprabasal Keratinocytes", "Intermediate Keratinocytes", "Basal Keratinocytes")

Dotplot_CO("SAT1", celllist)
Dotplot_AS("SAT1", celllist)
```
