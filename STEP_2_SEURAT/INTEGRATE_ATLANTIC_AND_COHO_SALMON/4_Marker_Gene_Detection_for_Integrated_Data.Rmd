---
title: "Marker Gene Detection for Integrated Data"
...
```{r load rds}
ASCO.integrated <- readRDS("ASCO.integrated.rds")
```
...
DefaultAssay(ASCO.integrated) <- "SCT"
```
...
ASCO.integrated <- PrepSCTFindMarkers(ASCO.integrated, assay = "SCT")
...
ASCO.markers <- FindAllMarkers(ASCO.integrated, assay = "SCT", slot = "data", only.pos = TRUE, min.pct = 0.25, return.thresh = 0.01, pseudocount.use = 0.001, logfc.threshold = 0.25, test.use = "LR", latent.vars = "orig.ident")

# save to file
write.table(ASCO.markers, file="DE-ASCO.txt", sep="\t")
```
